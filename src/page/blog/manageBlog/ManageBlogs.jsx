...
const ManageBlogs = () => {
    ...
    const navigate = useNavigate()
     useEffect(()=>{
            fetch('https://meta-blog-backend-6yio.vercel.app/blogs').then((res)=>res.json()).then((data)=>setblogs(data.blogs)).catch((error)=>console.error('error featching blog data :' + error))
        })
        const hadleblogDelete=async(id)=>{
            try {
                await axios.delete(`https://meta-blog-backend-6yio.vercel.app/blogs/${id}`)
